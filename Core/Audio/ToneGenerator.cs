...
public class ToneGenerator : IDisposable
{
    ...
    public ToneGenerator()
    {
        ...
        {
            Console.WriteLine($" Error inicializando audio: {ex.Message}");
            Console.WriteLine("Usando modo simulado (mostrará notas sin sonido).");
        }
    }
    ...
    public void PlayTone(double frequency, double durationSeconds)
    {
        ...
        lock (_lock)
        {
            try
            {
                PlayToneInternal(frequency, durationSeconds);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"  [ERROR] reproduciendo tono: {ex.Message}"); 
            }
        }
    }
