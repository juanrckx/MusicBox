...
namespace MusicBox.Tests
{
    public class AudioEngineTests
    {
        ...
        public void PlayScore_ForwardAndBackward_CanBeCalled()
        {
            // Arrange
            using var audioEngine = new AudioEngine();
            var doscore = new DoublyLinkedList<Note>();
            var rescore = new DoublyLinkedList<Note>();

            // Crear una nota simple
            doscore.AddLast(new Note
            {
                Name = NoteName.Do,
                Frequency = 261.63,
            ...
            var exception1 = Record.Exception(() => audioEngine.PlayScore(doscore, forward: true));
            var exception2 = Record.Exception(() => audioEngine.PlayScore(rescore, forward: false));
